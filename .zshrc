...
alias yt='noglob yt-dlp --output '\''$HOME/Downloads/yt-dlp/%(channel)s_%(channel_id)s/%(upload_date>%Y-%m-%d)s_%(title)s_%(duration>%H-%M-%S)s_%(resolution)s.%(ext)s'\'' --ffmpeg-location /home/jjenkx/.local/bin.notpath/ --restrict-filenames --external-downloader aria2c --downloader-args "aria2c: -s 32 -x 32 -j 8 -c -k 8K --piece-length=128K --lowest-speed-limit=10K --retry-wait=2 --continue=true " --write-description --write-info-json --write-thumbnail --prefer-free-formats --remux-video mkv --embed-chapters --sponsorblock-remove "sponsor,selfpromo,interaction,intro,outro,preview " --download-archive $HOME/Downloads/yt-dlp/.yt-dlp-archived-done.txt '
...
alias yp='noglob yt-dlp --exec '\''nohup mpv '\''%(filepath)q'\'' &>/dev/null & '\'' --output '\''$HOME/Downloads/yt-dlp/%(channel)s/%(upload_date>%Y-%m-%d)s_%(title)s/%(upload_date>%Y-%m-%d)s_%(title)s_%(duration>%H-%M-%S)s_%(resolution)s.%(ext)s'\'' --ffmpeg-location /home/jjenkx/.local/bin.notpath/ --restrict-filenames --external-downloader aria2c --downloader-args "aria2c: -s 32 -x 32 -j 8 -c -k 8K --piece-length=28K --lowest-speed-limit=10K --retry-wait=2 --continue=true " --write-description --write-info-json --write-thumbnail --prefer-free-formats --remux-video mkv --embed-chapters --sponsorblock-remove "sponsor,selfpromo,interaction,intro,outro,preview " --download-archive $HOME/Downloads/yt-dlp/.yt-dlp-archived-done.txt '
